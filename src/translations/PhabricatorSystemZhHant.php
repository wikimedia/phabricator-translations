...
final class PhabricatorSystemZhHant
  ...
  protected function getTranslations() {
    return array(
      ...
      'Destroying %s **%s**...
    ' => '正在銷毀%s**%s**…',
      'This server has consistently been unable to reach the writable ("master") database while processing recent requests.' => '在處理最近的請求時，Phabricator 一直無法接觸到可寫入資料庫（「master」）。',
      ...
      'This server was unable to connect to the writable ("master") database while handling this request, and automatically degraded into read-only mode.' => '在處理此請求時，Phabricator 無法連接到可寫入的資料庫（「master」）並自動降級成唯讀模式。',
    ...
    這些邊緣破損是銷毀物件的預期結果，Phabricator 的上游會無法幫助您修復它們。我們強烈建議以停用或歸檔物件的方式來取代。',
      ...
      'This may also indicate that a more serious failure has occurred. If this interruption does not resolve on its own, this server will soon detect the persistent disruption and degrade into read-only mode until the issue is resolved.' => '這也可能代表著是更嚴重的問題發生。若中斷無法自行解決，Phabricator 會在檢測到持續中斷出現後降級成唯讀模式，直到問題有被處理掉。',
      ...
      'This server will periodically retry the connection and recover once service is restored. Most causes of persistent service interruption will require administrative intervention in order to restore service.' => 'Phabricator 會定期重試連線，一旦服務可用後即可恢復。多數的服務持續中斷會需要管理員介入才能恢復。',
      ...
      'This may happen if there is a temporary network anomaly on the server side, like cosmic radiation or spooky ghosts. If this failure was caused by a transient service interruption, this server will recover momentarily.' => '如果在伺服器端那裡出現臨時網路異常，像是有宇宙輻射或是幽靈鬼魂，則可能會發生這種情況。若此故障是由一時服務中斷造成的，稍待一下 Phabricator 便會恢復。',
