...
final class PhabricatorSystemZhHant
  ...
  protected function getTranslations() {
    ...
    ' => '正在銷毀%s**%s**…',
      ...
      '%d / hour' => '%s/時',
      'This may happen if there is a temporary network anomaly on the server side, like cosmic radiation or spooky ghosts. If this failure was caused by a transient service interruption, Phabricator will recover momentarily.' => '如果在伺服器端那裡出現臨時網路異常，像是有宇宙輻射或是幽靈鬼魂，則可能會發生這種情況。若此故障是由一時服務中斷造成的，稍待一下 Phabricator 便會恢復。',
      'Rate Limiting Actions' => '操作限制速率',
      'Highlighting' => '凸顯中',
      'Unable to Reach Master' => '無法接觸到 Master',
      'Phabricator was unable to connect to the writable ("master") database while handling this request, and automatically degraded into read-only mode.' => '在處理此請求時，Phabricator 無法連接到可寫入的資料庫（「master」）並自動降級成唯讀模式。',
    ...
    These tattered edges are an expected consequence of destroying objects, and the Phabricator upstream will not help you fix them. We strongly recommend disabling or archiving objects instead.' => '銷毀物件可能會導致相關物件停止運作，並且可能會殘留對於已不存在物件的分散引用。在多數情況下，停用或歸檔物件會是比銷毀更好的方式。此風險會在刪除複雜或是高度連接物件（例如版本庫、專案、以及使用者）時最大。
    這些邊緣破損是銷毀物件的預期結果，Phabricator 的上游會無法幫助您修復它們。我們強烈建議以停用或歸檔物件的方式來取代。',
      'IMPORTANT' => '重要',
      ...
      'Read-only mode was enabled by the explicit action of a human administrator, so you can get more information about why it has been turned on by rolling your chair away from your desk and yelling "Hey! Why is Phabricator in read-only mode??!" using your very loudest outside voice.' => '唯讀模式是被人類管理員的明確操作行為給啟用，所以您可以從您的桌椅上跳開，用您最大的音量大叫「嘿！為什麼 Phabricator 是唯讀模式？？！」來獲得更多資訊。',
      'This mode is active because no database has a "%s" role in the configuration option "%s".' => '此模式目前有效，因為在設置選項「%2$s」中沒有資料庫具有「%1$s」身分。',
      'This may also indicate that a more serious failure has occurred. If this interruption does not resolve on its own, Phabricator will soon detect the persistent disruption and degrade into read-only mode until the issue is resolved.' => '這也可能代表著是更嚴重的問題發生。若中斷無法自行解決，Phabricator 會在檢測到持續中斷出現後降級成唯讀模式，直到問題有被處理掉。',
      ...
      'An administrator has placed Phabricator into read-only mode.' => '管理員已將 Phabricator 置於唯讀模式。',
      ...
      'Phabricator has consistently been unable to reach the writable ("master") database while processing recent requests.' => '在處理最近的請求時，Phabricator 一直無法接觸到可寫入資料庫（「master」）。',
      ...
      'System' => '系統',
      'You are performing too many actions too quickly.' => '您太快執行太多個操作。',
      'Phabricator will periodically retry the connection and recover once service is restored. Most causes of persistent service interruption will require administrative intervention in order to restore service.' => 'Phabricator 會定期重試連線，一旦服務可用後即可恢復。多數的服務持續中斷會需要管理員介入才能恢復。',
      ...
      'The maximum allowed rate for this action is %s. You are taking actions at a rate of %s.' => '此操作的最大允許速率為 %s。您正在以 %s 速率進行此操作。',
