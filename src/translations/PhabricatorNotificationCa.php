...
final class PhabricatorNotificationCa
  ...
  protected function getTranslations() {
    return array(
      'Unread' => 'Sense llegir',
      'Unable to load server status: this is not an admin server!' => 'No es pot carregar l\'estat del servidor: això no és un servidor d\'administrador!',
      'All Notifications' => 'Totes les notificacions',
      'Really mark all notifications as read?' => 'Marcar totes les notificacions com a llegides?',
      'Notification server configuration is invalid: it does not specify any enabled servers with type "admin". Notifications require at least one active "admin" server.' => 'La configuració del servidor de notificacions no és vàlida: no especifica cap servidor habilitat amb tipus "admin". Les notificacions requereixen com a mínim un servidor "admin" actiu.',
      'Setting Up Client' => 'Configurant el client',
      'Notification server configuration is invalid: it does not specify any enabled servers with type "client". Notifications require at least one active "client" server.' => 'La configuració del servidor de notificacions no és vàlida: no especifica cap servidor habilitat amb tipus "client". Les notificacions requereixen com a mínim un servidor "client" actiu.',
      'Phabricator is configured to use a notification server, but is not able to connect to it.' => 'Phabricator està configurat per utilitzar un servidor de notificacions, però no pot connectar-s\'hi.',
      'All unread notifications will be marked as read. You can not undo this action.' => 'Totes les notificacions no llegides seran marcades com a llegides. Aquesta acció no es pot desfer.',
      ...
      '%s notification(s) about objects which no longer exist or which you can no longer see were discarded.' => '%s notificacions sobre objectes que ja no existeixen o que ja no podeu veure s\'han descartat.',
      'Notification server configuration describes an invalid host ("%s", at index "%s") with an unrecognized type ("%s"). Valid types are "%s" or "%s".' => 'La configuració del servidor de notificacions descriu un amfitrió no vàlid ("%s", a l\'índex "%s") amb un tipus no reconegut ("%s"). Els tipus vàlids són "%s" o "%s".',
      'Notification server configuration describes an invalid host ("%s", at index "%s") with an invalid protocol ("%s"). Valid protocols are "%s" or "%s".' => 'La configuració del servidor de notificacions descriu un amfitrió no vàlid ("%s", a l\'índex "%s") amb un protocol no vàlid ("%s"). Els protocols vàlids són "%s" o "%s".',
      'No notifications to mark as read.' => 'No hi ha notificacions per marcar com a llegides.',
      'Notification Server Down' => 'Servidor de notificacions caigut',
      'Unable to test client on an admin server!' => 'No es pot provar el client en un servidor administrador!',
      'Notification server configuration describes an invalid host ("%s", at index "%s"). This is an "admin" service but it has a "path" property. This property is only valid for "client" services.' => 'La configuració del servidor de notificacions descriu un amfitrió no vàlid ("%s", a l\'índex "%s"). Això és un servei "administrador" però té una propietat "ruta". Aquesta propietat només és vàlida per serveis "client".',
    ...
    %s',
      'Notification server not enabled' => 'Servidor de notificacions no habilitat',
      'This is a test notification, sent at %s.' => 'Això és una notificació de prova, s\'ha enviat a %s.',
      '(To start the server, run this command.)
    %s' => '(Per iniciar el servidor, executa aquesta ordre.)
    %s',
      ...
      'Mark All Read' => 'Marca-ho tot com llegit',
