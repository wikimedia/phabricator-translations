...
final class PhabricatorNotificationZhHans
  ...
  'Notification server configuration describes an invalid host ("%s", at index "%s") with an unrecognized type ("%s"). Valid types are "%s" or "%s".' => '通知服务器的配置中存在以无法识别的类型（“%3$s”）描述的无效主机（“%s”，位于索引“%s”）。有效的类型应当为“%4$s”或“%5$s”。',
  'Notification server configuration describes an invalid host ("%s", at index "%s") with an invalid protocol ("%s"). Valid protocols are "%s" or "%s".' => '通知服务器的配置中存在以无法识别的协议（“%3$s”）描述的无效主机（“%s”，位于索引“%s”）。有效的协议应当为“%4$s”或“%5$s”。',
...
%s' => '此服务器被配置为使用通知服务器，但无法连接到它。您应该尝试解决此问题或禁用通知服务器。建议您仔细检查您的配置，或使用下面的命令来重新启动服务器。
