...
final class PhabricatorConfigFrFR
  ...
  protected function getTranslations() {
    ...
    Cette valeur en base de données est actuellement respectée, mais une future version de Phabricator cessera de respecter les valeurs en base de données pour les options de configuration verrouillées.',
    ...
    Les requêtes doivent inclure un entête « Host » valide.',
      ...
      'The request body that was sent began:' => 'Le corps de la requête envoyé par Phabricator commençait par :',
    ...
    Après avoir activé les prototypes, vous pouvez les désinstaller unitairement (comme des applications normales).',
      ...
      'Table' => 'Tableau',
      'The best available MYSQL implementation is now selected automatically.' => 'Phabricator sélectionne dorénavant automatiquement la meilleure mise en œuvre disponible de MySQL.',
    ...
    Vous devez activer DarkConsole en définissant « %s » avant que cette option ait un quelconque effet.',
      'Alternative URIs that can access this service.' => 'Uri alternatives qui peuvent accéder à Phabricator.',
      ...
      'This server is not configured in cluster mode.' => 'Phabricator n’est pas configuré en mode grappe.',
    ...
    Vous devriez pouvoir installer l’extension avec une commande comme : %s',
      'The current configuration has these %d value(s):' => 'La configuration actuelle de Phabricator a ces %s valeurs :',
    ...
    Si vous avez configuré la synchronisation automatiquement des informations de compte depuis un autre système faisant autorité, vous pouvez empêcher les utilisateurs de faire ces modifications pour être sûr que les informations restent cohérentes entre les deux systèmes.',
      ...
      'You have \'%s\' enabled in your PHP configuration.
    This option is not compatible with this software. Disable \'%s\' in your PHP configuration to continue.' => 'Vous avez « %s » d’activé dans votre configuration PHP.
    Cette option n’est pas compatible avec ce logiciel. Désactivez « %s » dans votre configuration PHP pour continuer.',
    ...
    Les fonctionnalités de gestion de signaux nécessaires ont été retirées dans PHP 7.0 et le langage n\'a pas restauré de mécanisme équivalent jusqu\'à PHP 7.1.',
      'You haven\'t configured mailers yet, so this server won\'t be able to send outbound mail or receive inbound mail. See the configuration setting "cluster.mailers" for details.' => 'Вы еще не настроили почтовые программы, поэтому вы не сможете отправлять исходящую почту или получать входящую почту.  См. Параметр конфигурации cluster.mailers для подробностей.',
    ...
    %s',
      ...
      'Identify the component in your webserver configuration which is decompressing or mangling requests and disable it. This software will not work properly until you do.' => 'Identifiez le composant de la configuration du serveur web qui décompresse ou embrouille les requêtes et désactivez-le. Phabricator ne fonctionnera pas correctement tant que vous ne le ferez pas.',
      ...
      'The \'%s\' extension is not installed. Without \'%s\', support, this software may not be able to determine the MIME types of uploaded files.' => 'L’extension « %s » n’est pas installée. Sans la prise en charge de « %s », Phabricator pourrait ne pas pouvoir déterminer les types MIME des fichiers téléversés.',
      'Stop this software from sending any email, etc.' => 'Empêcher Phabricator d\'envoyer des courriels, etc.',
      'The \'%s\' binary could not be found. Symlink it into \'%s\', or set the webserver\'s %s environmental variable to include the directory where it resides, or add that directory to \'%s\' in configuration.' => 'Le binaire \'%s\' n’a pas pu être trouvé. Faire un lien symbolique dessus dans \'%s\', ou initialiser la variable d’environnement %s du serveur web pour inclure le répertoire où il réside, ou ajouter ce répertoire à \'%s\' dans la configuration de Phabricator.',
      ...
      'Configure services to run on a cluster of hosts.' => 'Configurer Phabricator pour s’exécuter sur une grappe d’hôtes.',
    ...
    %s',
      ...
      'Configure the UI, including colors.' => 'Configurer l’IHM de Phabricator, y compris les couleurs.',
      ...
      'Control how user names are rendered in mail.' => 'Contrôler comment Phabricator formate les noms d’utilisateur dans les courriels.',
    ...
    Après avoir exécuté `bin/auth unlock` et effectué vos changements sur la configuration du fournisseur d’authentification, vous devriez exécuter `bin/auth lock`.',
      ...
      'Storage engines are now discovered automatically at runtime.' => 'Phabricator découvre maintenant de façon automatique les moteurs de stockage disponibles à l’exécution.',
      'This software appears to be installed on a very small EC2 instance (of class "%s") with burstable CPU. This is strongly discouraged. This software regularly needs CPU, and these instances are often choked to death by CPU throttling. Use an instance with a normal CPU instead.' => 'Phabricator semble être installé sur une très petite instance d’EC2 (de classe « %s ») qui utilise un processeur virtuel « extensible » qui n’effectue ses traitements que par rafales. Ceci est fortement déconseillé. Phabricator a besoin du processeur de façon régulière et ces instances sont souvent chargées à mort par la limitation et le partage d’usage du processeur physique. Utilisez plutôt une instance avec un processeur normal.',
      ...
      'Daemons no longer use PID files.' => 'Les services d’arrière-plan de Phabricator n’utilisent plus les fichiers PID.',
    ...
    (Ce problème peut provenir d’un « B » manquant dans votre RewriteRule.)',
      ...
      'Global access controls now exist, see `%s`.' => 'Phabricator a maintenant des contrôles d’accès globaux importants. Voir `%s`.',
    ...
    Assurez-vous que Phabricator et MySQL sont bien configurés.',
      ...
      'You can provide a custom highlighter engine by extending class %s.' => 'Phabricator peut mettre en évidence le PHP par défaut et utiliser Pygments pour d’autres langages, s’il est activé. Vous pouvez fournir un moteur de mise en évidence personnalisé en étendant la classe %s.',
    ...
    If not set, no log will be written.' => 'Pour activer le journal des erreurs SSD de Phabricator, spécifiez un chemin. Les erreurs qui se produisent lorsque Phabricator sert des requêtes SSH seront inscrites dans ce journal.
    ...
    To learn more about how this issue affects this software, see %s.' => 'La version de %s sur ce système est obsolète et contient une vulnérabilité majeure largement connue (la faille « Shellshock »).
    ...
    Votre serveur web peut ne pas être configuré pour faire suivre l’authentification HTTP basique. Si vous prévoyez d’utiliser l’authentification basique (par exemple, pour accéder aux dépôts), vous devez le reconfigurer.',
      ...
      'Controls whether email is sent "From" users.' => 'Contrôle si Phabricator envoie les courriels « de la part » des utilisateurs.',
    ...
    Nous vous recommandons fortement de vous mettre à jour vers MySQL 5.5 ou plus récent.',
      ...
      'PHP 7 Compatibility Information' => 'Information de compatibilité PHP 7 de Phabricator',
    ...
    Le compte utilisateur sous lequel tourne le serveur web doit pouvoir lire tous les répertoires dans PATH afin de les utiliser.',
      ...
      'You can rebuild repository identities while the server is running.' => 'Vous pouvez reconstruire les identités de dépôt pendant l’exécution de Phabricator.',
    ...
    (If you already created tables, MySQL incorrectly used some other engine to create them. You need to convert them or drop and reinitialize them.)' => 'Le moteur « InnoDB » n’est pas disponible dans MySQL (sur la machine « %s »). Activez InnoDB dans votre configuration MySQL.
    ...
    Une fois que vous avez installé et activé Pygments, le code source nouvellement créé (comme les diffs et les collages) devraient être mis en évidence correctement. Vous devrez vider les caches de Phabricator pour que le code source déjà existant soit mis en évidence. Pour les instructions concernant la gestion des caches, voyez [[%s|Gérer les caches]].',
      ...
      'The namespace that databases should use.' => 'L’espace de noms que les bases de données de Phabricator doivent utiliser.',
      ...
      'This software sent itself a test request that was compressed with "Content-Encoding: gzip", but received different bytes than it sent.' => 'Phabricator s’est envoyé à lui-même une requête de test qui a été compressée avec « Content-Encoding: gzip », mais il a reçu des octets différents de ceux qu’il a envoyés.',
      ...
      'The configuration option "%s" is not recognized. It may be misspelled, or it might have existed in an older version of the software. It has no effect, and should be corrected or deleted.' => 'L’option de configuration \'%s\' n’est pas reconnue. Elle est peut-être mal écrite, ou peut avoir existé dans une ancienne version de Phabricator. Elle n’a pas d’effet, et doit être corrigée ou supprimée.',
      'You can rebuild the search index while the server is running.' => 'Vous pouvez reconstruire l\'index de recherche pendant l\'exécution de Phabricator.',
      ...
      'Daemons Are Not Running' => 'Pas de démons Phabricator en cours d\'exécution',
      ...
      'The request body that was received began:' => 'Le corps de la requête reçue par Phabricator commençait par :',
    ...
    Après avoir modifié cette valeur, vous devez redémarrer les démons. La plupart des modifications de configuration sont automatiquement prises en compte par les démons, mais la taille des jeux de ressources ne peut pas être modifiée sans redémarrage de chaque instance.',
      ...
      'Your version of MySQL (on database host "%s") does not support configuration of a stopword file. You will not be able to find search results for common words.' => 'Votre version de MySQL (sur l’hôte de base de données « %s ») ne prend pas en charge la configuration d’un fichier de mots vides. Vous pourriez ne pas trouver de résultats de recherche pour des mots communs.',
    ...
    %s',
      'Your configuration fails to specify a server timezone. You can either set the PHP configuration value \'%s\' or the %s configuration value \'%s\' to specify one.' => 'Votre configuration échoue à spécifier un fuseau horaire de serveur. Vous pouvez soit définir la valeur de configuration PHP « %s » ou la valeur de configuration « %s » de Phabricator pour en spécifier une.',
      'Enable HTTP Strict Transport Security (HSTS).' => 'Activer la Sécurité de Transport HTTP Stricte (HSTS).',
      'This server has %s available in %s, but the binary exited with an error code when run as %s. Check that it is installed correctly.' => 'Phabricator a %s disponible dans %s, mais le binaire s’est terminé avec un code erreur quand il est exécuté comme %s. Vérifiez qu’il est installé correctement.',
      ...
      'Transaction mail is now always sent with "Precedence: bulk" to improve deliverability.' => 'Phabricator envoie maintenant toujours un courriel de transaction avec « Precedence:bulk » pour améliorer la capacité de livraison.',
      'Retention Policy' => 'Politique de rétention',
      'Without \'%s\', this software can not test for the availability of other binaries.' => 'Sans \'%s\', Phabricator ne peut pas tester la disponibilité d’autres binaires.',
      ...
      'Do not install this software on an instance class with burstable CPU.' => 'Ne pas installer Phabricator sur une classe d’instance avec un CPU extensible.',
      'Real' => 'Réel',
    ...
    Si vous êtes certain d’avoir une version récente de « %s » installée et fonctionnant correctement, il est en général sans problème d’ignorer cet avertissement.',
      ...
      'Set the URI where this software is installed. Setting this improves security by preventing cookies from being set on other domains, and allows daemons to send emails with links that have the correct domain.' => 'Définir l’URI où Phabricator est installé. Le définir améliore la sécurité en évitant que les cookies soient définis sur d’autres domaines, et permet aux démons d’envoyer des courriels avec des liens qui ont le bon domaine.',
    ...
    Si le paramètre actuel vous satisfait, vous pouvez sans souci ignorer cet avertissement d’installation.',
      ...
      'Set a string this software should use to prefix cookie names.' => 'Mettre une chaîne que Phabricator doit utiliser pour préfixer les noms de cookie.',
    ...
    Quelque chose est mal configuré ou sinon, déforme les réponses.',
      ...
      'The timezone this software should use by default.' => 'Le fuseau horaire que Phabricator doit utiliser.',
    ...
    La valeur actuelle de « PATH » après application de la configuration est :
      ...
      'Reply hints are no longer shown in mail.' => 'Phabricator ne montre plus les conseils de réponse dans le courriel.',
      ...
      'Typeahead strategies are now managed automatically.' => 'Phabricator gère maintenant automatiquement les stratégies de saisie anticipée.',
    ...
    To resolve this issue, remove all URIs from "feed.http-hooks".' => 'L\'option « feed.http-hooks » a été rendue obsolète au profit des crochets web. Elle sera supprimée dans une version future de Phabricator.
    Vous pouvez configurer les crochets web dans Herald.
    Pour résoudre ce problème, supprimez toutes les URI de « feed.http-hooks ».',
      ...
      'Database host "%s" is configured as a master, but is replicating another host. This is dangerous and can mangle or destroy data. Only replicas should be replicating. Stop replication on the host or adjust configuration.' => 'L’hôte de base de données « %s » est configuré comme maître, mais réplique un autre hôte. C’est dangereux, et peut déformer ou détruire des données. Seuls les répliques devraient être répliquées. Arrêtez la réplication sur l’hôte, ou reconfigurez Phabricator.',
    ...
    When cluster addresses are defined, hosts will also reject requests to interfaces which are not whitelisted.' => 'Définir une grappe Phabricator en fournissant une liste blanche d’adresses d’hôtes qui font partie de la grappe.
    ...
    NOTE : Vous devez installer XHProf pour que les profils fonctionnent.',
      ...
      'Available search engines are now automatically discovered at runtime.' => 'Phabricator découvre maintenant automatiquement les moteurs de recherche au moment de l\'exécution.',
      ...
      'The environmental variable %s is empty. This server will not be able to execute some commands.' => 'La variable d’environnement %s est vide. Phabricator ne pourra pas exécuter certaines commandes.',
      '%s.%s (%s)' => '%s.%s (%s)',
    ...
    Vous devriez configurer un CDN ou un domaine de fichier alternatif pour réduire ce risque. Configurer un CDN améliorera aussi les performances. Voyez [[ %s | %s ]] pour des instructions à ce sujet.',
      ...
      'You must start the daemons to send email, rebuild search indexes, and do other background processing.' => 'Vous devez démarrer les démons de Phabricator pour envoyer des courriels, reconstruire les index de recherche, et faire d’autres traitements en tâche de fond.',
      ...
      'PHP 7.0-7.1 Not Supported' => 'PHP 7.0-7.1 non pris en charge',
      ...
      'Global customization of monospaced fonts is no longer supported.' => 'Phabricator ne prend plus en charge la personnalisation globale des polices à chasse fixe.',
      ...
      'This server received an "X-Mod-Pagespeed" or "X-Page-Speed" HTTP header on this request, which indicates that you have enabled "mod_pagespeed" on this server. This module is not compatible with this software. You should disable the module.' => 'Phabricator a reçu un entête HTTP « X-Mod-Pagespeed » ou « X-Page-Speed » pour cette requête, ce qui indique que vous avez activé « mod_pagespeed » sur ce serveur. Ce module n’est pas compatible avec Phabricator. Vous devriez le désactiver.',
      ...
      'Customized settings for applications.' => 'Paramètres personnalisés pour les applications Phabricator.',
      ...
      'Controls whether email for multiple recipients is sent by creating one message with everyone in the "To:" line, or multiple messages that each have a single recipeint in the "To:" line.' => 'Contrôle si Phabricator envoie un courriel avec plusieurs destinataires dans la ligne « À », ou plusieurs courriels séparés avec pour chacun un unique destinataire sur la ligne « À ».',
    ...
    messages.' => 'Vous pouvez désactiver les pieds de page des courriels « À : » et « Cc : » si les utilisateurs préfèrent des messages plus petits.',
    ...
    production instance were both in operation.' => 'Cette option vous permet d’arrêter l’envoi de données par Phabricator à la plupart des services
    externes : ceci désactivera les courriels, les SMS, la mise en miroir de dépôts, les constructions
    distantes, les écritures Doorkeeper et les crochets web.
    Cette option vise à permettre à une instance de Phabricator d’être exportés, copiée ou importée
    et de s’exécuter dans un environnement de test sans impacter les utilisateurs. Par exemple, si
    vous faites une migration vers un nouveau matériel, vous pouvez effectuer un test de migration
    avec d’abord ce drapeau mis, vous assurer que les choses fonctionnent, puis faire le basculement
    de production plus tard avec plus de confiance et moins de perturbation.
    Sans l’utilisation de ce drapeau pour rendre silencieux l’environnement temporaire de test, les
    utilisateurs recevraient des courriels en doublon durant la période où l’instance de test et l’ancienne
    instance de production étaient toutes les deux en fonctionnement.',
    ...
    Pour configurer l’URI de base, lancez la commande affichée ci-dessous.',
      'The mapping from VCS users to %s users has changed and must be rebuilt.' => 'Отображение от пользователей VCS к пользователям Phabricator изменилось и должно быть восстановлено.',
    ...
    Les requêtes doivent inclure un entête « Host » valide.',
      ...
      'Sets the default color scheme.' => 'Mettre le schéma de couleur par défaut de Phabricator.',
      'Clustering Introduction' => 'Introduction de la mise en grappe',
      'By default, this software includes some flavor text in the UI, like a prompt to "Weigh In" rather than "Add Comment" in Maniphest. If you\'d prefer more traditional UI strings like "Add Comment", you can set this flag to disable most of the extra flavor.' => 'Par défaut, Phabricator inclut certaines variantes de texte dans l’IHM, comme une invite à « Intervenir » plutôt que « Ajouter un commentaire » dans Maniphest. Si vous préférez des chaînes d’IHM plus traditionnelles comme « Ajouter un commentaire », vous pouvez positionner ce drapeau pour désactiver la plupart des variantes supplémentaires.',
    ...
    Cela signifie en général que vous avez modifié un fichier et laissé des caractères blancs avant la balise d’ouverture %s ou après la balise de fermeture %s. Retirez tout espacement en excès au début et omettez de préférence les balises de fermeture.',
      ...
      'You can usually install a PHP extension using %s, %s, or %s. A common package name is %s. Try commands like these:' => 'Vous pouvez en général installer une extension PHP en utilisant %s ou %s. Les noms de paquet courants sont %s ou %s. Essayez des commandes comme :',
    ...
    [[ %s | PHP List of Supported Timezones ]].' => 'Les fonctions de date de PHP émettront un avertissement si elles sont appelées quand
    aucun fuseau horaire de serveur par défaut n’est configuré.
    Habituellement, vous configurez un fuseau horaire par défaut dans `php.ini` en définissant
    la valeur de configuration `date.timezone`.
    Si vous préférez, vous pouvez configurer un fuseau horaire par défait à la place. Pour
    configurer un fuseau horaire par défaut, sélectionnez un de ceux de la
    [[ %s | Liste des fuseaux horaires pris en charge dans PHP ]].',
    ...
    If you provide an instance identifier here (normally by injecting it with a `%s`), the server will pass it to subprocesses and commit hooks in the `%s` environmental variable.' => 'AVERTISSEMENT : ceci est une option très avancée, utile seulement aux fournisseurs d’hébergement exécutant des grappes pour locataires multiples.
    Si vous fournissez un identifiant d’instance ici (normalement en l’injectant avec un `%s`), Phabricator le passera aux sous-processus et validera les crochets dans la variable d’environnement `%s`.',
      ...
      'Arbitrarily large files are now supported. Consult the documentation for configuration details.' => 'Phabricator prend maintenant en charge des fichiers arbitrairement gros. Consulter la documentation pour les détails sur la configuration.',
