...
final class PhabricatorTransactionsZhHans
  ...
  'In call to "transaction.search", selected object (of type "%s") does not implement "%s", so transactions can not be loaded for it.' => '在调用“transaction.search”时，选定的对象（类型为“%s”）未实现“%s”，因此无法为其加载交易。',
  ...
  '%s removed %s subscriber(s) for %s: %s.' => '%s移除了%3$s的%s个订阅者：%4$s。',
  ...
  'Read transactions and comments for a particular object or an entire object type.' => '读取特定对象或整个对象类型的交易和评论。',
  '%s rescinded a token.' => '%s取消了一个令牌。',
  '%s awarded a token.' => '%s授予了一个令牌。',
  'Edit Form %d: %s' => '编辑表单%d：%s',
  '%s removed %d subscriber(s): %s.' => '%s移除了%d个订阅者：%s。',
  ...
  'Calls to "transaction.search" must specify either an "objectType" or an "objectIdentifier".' => '对“transaction.search”的调用必须指定“objectType”或“objectIdentifier”。',
  'Forms' => '词形',
  'Mention' => '提及',
  'export' => '出口',
  'Post Comment' => '发布评论',
  ...
  '%s created %s in the %s space.' => '%s在%3$s空间中创建了%s。',
  '%s added this form to the "Create" menu.' => '%s将此表单添加至“创建”菜单。',
  'No Edit Forms' => '没有编辑表单',
  '%s added %s unsubscriber(s) for %s: %s.' => '%s为%3$s添加了%s个订阅者：%4$s。',
  'Calls to "transaction.search" must not specify both an "objectType" and an "objectIdentifier".' => '对“transaction.search”的调用不能同时指定“objectType”和“objectIdentifier”。',
  'Edit Defaults' => '编辑默认值',
  ...
  '%s added %d subscriber(s): %s.' => '%s添加了%d个订阅者：%s。',
  'In call to "transaction.search", specified "objectIdentifier" ("%s") does not exist.' => '调用“transaction.search”时，指定的“objectIdentifier”（“%s”）不存在。',
  ...
  '%s moved this task from %s to %s on the %s board.' => '%s在%4$s版块上，将此任务从%s移动到%s。',
  ...
  '%s edited subscriber(s), added %d: %s; removed %d: %s.' => '%s编辑了订阅者，添加了%d：%s；移除了%d：%s。',
  ...
  'Parameter "%s" must contain a list of transaction descriptions, but item with key "%s" is not a dictionary.' => '参数“%s”必须含有交易描述列表，但键为“%s”的项不是字典。',
  'Edit Comment' => '编辑评论',
  ...
  'Subtype "%s" is not valid: subtype keys may only contain lowercase latin letters ("a" through "z").' => '子类型“%s”无效：子类型键只能含有小写拉丁字母（a到z）。',
  ...
  'In call to "transaction.search", specified "objectType" ("%s") is unknown. Valid object types are: %s.' => '调用“transaction.search”时，指定的“objectType”（“%s”）未知。有效的对象类型包括：%s。',
