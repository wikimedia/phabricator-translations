...
//You will still receive some administrative email, like password reset email.//' => '**E-posta Bildirimleri**\'ni devre dışı bırakırsanız, bu sunucu etkinlikler hakkında sizi bilgilendirmek için asla e-posta göndermez. Bu tercih, diğer tüm ayarlarınızı geçersiz kılar.
...
Bu, postayı daha kullanışlı hale getirebilir, ancak bazı istemciler bu mesajları işlerken zorlanırlar. Bu seçeneğin devre dışı bırakılması, konu satırlarını daha az kullanışlı hale getirme pahasına iletiyi geliştirebilir.',
  ...
  'If you change your primary address, %s will send all email to %s.' => 'Birincil adresinizi değiştirirseniz, %s tüm e-postayı %s gönderir.',
  ...
  'DarkConsole is a debugging console for developing and troubleshooting applications. After enabling DarkConsole, press the {nav `} key on your keyboard to toggle it on or off.' => 'DarkConsole, uygulamalarını geliştirmek ve sorun gidermeye yönelik bir hata ayıklama konsoludur. DarkConsole\'u etkinleştirdikten sonra, açıp kapatmak için klavyenizdeki {nav `} tuşuna basın.',
...
Bu tercihler **yalnızca** bağlı olduğunuz nesneler için geçerlidir (örneğin, bir gözden geçirici olduğunuz Revizyonlar veya CC’de bulunduğunuz görevler). Başka nesneler oluşturulduğunda e-posta uyarıları almak için, [[ /herald/ | Herald Kuralları ]] yapılandır.',
  ...
  'You can opt to receive plain text email instead of HTML email. Plain text email works better with some clients.' => 'HTML e-postası yerine düz metin e-posta almayı seçebilirsiniz. Düz metin e-posta bazı müşterilerle daha iyi çalışır.',
  ...
  'Your browser has granted this server permission to send desktop notifications.' => 'Tarayıcınız bu sunucuya masaüstü bildirimleri gönderme izni verdi.',
  'This browser has denied permission to send desktop notifications to this server. Consult your browser settings / documentation to figure out how to clear this setting, do so, and then re-visit this page to grant permission.' => 'Bu tarayıcı masaüstü bildirimlerinin bu sunucuya gönderilmesine izin vermedi. Bu ayarın nasıl temizleneceğini bulmak için tarayıcı ayarlarınıza / belgelerinize bakın, yapın ve ardından izin vermek için bu sayfayı tekrar ziyaret edin.',
  'Week Starts On' => 'Hafta Başlamasına',
  'When a revision is updated, this software attempts to bring inline comments on the older version forward to the new changes. You can disable this behavior if you prefer comments stay anchored in one place.' => 'Bir revizyon güncellendiğinde, bu yazılım eski versiyondaki satır içi yorumları yeni değişikliklere ilerletmeye çalışır. Yorumların bir yerde sabit kalmasını tercih ederseniz bu davranışı devre dışı bırakabilirsiniz.',
...
See the tables below for a list of supported variables and protocols.' => '',
  ...
  'This server can send real-time notifications to your web browser or to your desktop. Select where you want to receive these real-time updates.' => 'Bu yazılım, web tarayıcınıza veya masaüstünüze gerçek zamanlı bildirimler gönderebilir. Bu gerçek zamanlı güncellemeleri nereden almak istediğinizi seçin.',
