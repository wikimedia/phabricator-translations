...
final class PhabricatorAphlictZhHant
  ...
  'No `%s` or `%s` binary was found in %s. You must install Node.js to start the Aphlict server.' => '在 %3$s 裡找不到 `%s` 或 `%s` 二進位檔案。您必須要安裝 Node.js 來啟動 Aphlict 伺服器。',
  ...
  'Failed to create directory "%s" for specified log file (with index "%s"). You should manually create this directory or choose a different logfile location. %s' => '未指定的日誌檔案（帶有索引「%2$s」）建立目錄「%s」失敗。您應手動建立目錄，或挑選不同的日誌檔案位置。%3$s',
  ...
  'Configuration file specifies cluster peer ("%s", at index "%s") with an invalid protocol, "%s". Valid protocols are "%s" or "%s".' => '設置檔案以無效的傳輸協定「%3$s」指定了叢集點（在索引「%s」，位於埠號「%s」）。有效的傳輸協定為「%4$s」或「%5$s」。',
