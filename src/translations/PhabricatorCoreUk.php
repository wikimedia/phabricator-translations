...
' => '**nuance** __command__ [__options__]
 ...
 Імпорт символів репозиторію (символи зчитуються зі stdin).',
  ...
  'Loading %s symbols...' => array(
    'Завантаження %s символи...',
    'Завантаження %s символів...',
  ),
...
Дані зазвичай прикріплюються шляхом виклику відповідного методу %s у класі Query під час завантаження об’єкта. Ви також можете явно викликати відповідний метод %s .',
  ...
  'Worker has no task ID.' => 'Робітник не має ідентифікатора завдання.',
  'Complete' => 'Завершений',
  ...
  'Visible' => 'Видимо',
  'Undocumented Install' => 'Недокументована Встановлення',
  'Exception' => 'Виняток',
  'Enable' => 'Увімкнути',
...
Якщо ви об\'єднаєте цю функцію з іншими обмеженнями, будуть повернуті результати, які або не мають значення або мають точно визначені значення.',
  ...
  'Object' => 'Об\'єкт',
  'Simple Subtypes' => 'Прості Підтипи',
...
Коротко:
  ...
  'You are logged in as %s.' => 'Ви ввійшли як %s.',
  'During the last %s second(s) spent waiting for the lock, %s other process(es) acquired it, so this is likely a bottleneck. Use "bin/lock log --name %s" to review log activity.' => array(
    'Протягом останніх секунди очікування блокування, %2$s інших процесів отримали його, тому це, ймовірно, уповільнить цей процес. Використайте "bin/lock log --name %3$s" для перегляду активності журналу.',
    'Протягом останніх %s секунд очікування блокування, %s інших процесів отримали його, тому це, ймовірно, уповільнить цей процес. Використайте "bin/lock log --name %s" для перегляду активності журналу.',
  ),
  'Supported commands are: %s.' => 'Підтримувані команди: %s.',
  ...
  'Projects' => 'Проєкти',
  'Pink' => 'Рожевий',
  'Provide a command to execute. This makes testing this script easier. When running normally, the command is read from the environment (%s), which is populated by sshd.' => 'Надайте команду для виконання. Це спрощує тестування цього сценарію. Під час нормального виконання команда зчитується з середовища (%s), яке заповнено sshd.',
  'GitHub' => 'GitHub',
...
' => '**fact** __command__ [__options__]
   Керування та налагодження вилучення даних,      сховищ та налаштувань, що використовуються для обчислення статистики.',
