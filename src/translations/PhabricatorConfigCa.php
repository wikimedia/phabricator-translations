...
final class PhabricatorConfigCa
  ...
  protected function getTranslations() {
    return array(
      'Review and modify configuration settings.' => 'Reviseu i modifiqueu la configuració de configuració.',
      ...
      'Column/Key' => 'Columna/Tecla',
      'Customize the logo and wordmark text in the header.' => 'Personalitzeu el text del logotip i la marca de paraules a la capçalera.',
    ...
    You must enable DarkConsole by setting \'%s\' before this option will have any effect.' => 'Aquesta opció us permet habilitar DarkConsole en cada pàgina, fins i tot per a usuaris que han tancat la sessió.  Això només és realment útil si necessiteu depurar alguna cosa en una pàgina de sortida.  No hauríeu d\'activar aquesta opció en la producció. <br><br>Heu d\'activar DarkConsole establint «%s» abans que aquesta opció tingui cap efecte.',
      ...
      'Configure Phabricator to run on a cluster of hosts.' => 'Configureu el Phabricator per executar-lo en un grup d\'amfitrions.',
    ...
    Si configureu el Phabricator en mode clúster, tingueu en compte que aquest ajustament és ignorat per sol·licituds intracluster.',
    ...
    (This problem can be caused by a missing "B" in your RewriteRule.)' => 'Phabricator es va enviar una sol·licitud de prova amb un camí inusual, per provar si el vostre servidor web està reescrivint els camins correctament.  El camí no s\'ha transmès correctament. <br><br>Phabricator va enviar una sol·licitud al camí "%s", i esperava que el servidor web decodiqués i reescrivís aquest camí de manera que rebés una petició de "%s".  No obstant això, va rebre una petició de "%s". <br><br>Verifica que les regles de reescriptura estan correctament configurades, seguint les instruccions de la documentació.  Si la codificació del camí no funciona correctament no podreu accedir als fitxers amb noms inusuals als repositoris, entre altres qüestions. <br><br>(Aquest problema pot ser causat per una "B" que manca en el vostre RewriteRule.)',
    ...
    %s' => 'S\'ha eliminat l\'opció de configuració «%s».  Podeu suprimir-ho segons convingui. <br><br>%s',
      ...
      'The minimum supported version of Mercurial is 1.9, which was released in 2011.' => 'La versió mínima suportada de Mercurial és 1.9, que va ser alliberada el 2011.',
      'Format for the SSH access log. Use %s to set the path. Available variables are:' => 'Format per al registre d\'accés SSH.  Utilitzeu %s per establir el camí.  Les variables disponibles són:',
    ...
    No activeu aquesta opció si publiqueu (o teniu previst publicar alguna vegada) contingut no segur mitjançant HTTP senzill. És molt difícil desfer aquest canvi un cop el navegador dels usuaris ha acceptat la configuració.',
    ...
    En aquests casos, podeu utilitzar aquesta opció per forçar que la pàgina interrompi després d\'un nombre més petit de segons (per exemple, 10), i bolcar una traça de pila útil. Això pot proporcionar informació útil sobre per què una pàgina que s\'ha quedat penjanda. 
    ...
    Disable safe mode to continue.' => 'Teniu activat \'%s\' a la configuració de PHP, però Phabricator no s\'executarà en mode segur. El mode segur ha quedat obsolet a PHP 5.3 i s’ha eliminat a PHP 5.4.',
      ...
      'This schema has the wrong nullable setting.' => 'Aquest esquema té la configuració anul·lable incorrecta.',
    ...
    Després d\'executar `bin/auth unlock` i fer els canvis a la configuració del proveïdor d\'autenticació, haureu d\'executar `bin/auth lock`.',
      ...
      'Phabricator now manages typeahead strategies automatically.' => 'Phabricator gestiona ara semiautomàticament les estratègies de pre-entrada.',
    ...
    Requests must include a valid "Host" header.' => 'Aquesta sol·licitud incloïa una capçalera "Amfitrió" no vàlida, amb el valor "%s". Les capçaleres d\'amfitrió han de contenir un punt (\'.\'), Com ara \'example.com\'. Això és necessari perquè alguns navegadors puguin configurar cookies.
    Això pot significar que l\'URI de base s\'ha configurat incorrectament. Heu de publicar Phabricator des d\'un URI base amb un punt (com ara "https://phabricator.mycompany.com"), no un domini (com ara "https://phabricator.mycompany.com"). Si esteu provant d\'utilitzar un domini senzill, canvieu la configuració per utilitzar un domini complet amb un punt.
    Això també pot significar que el vostre servidor web (o algun altre dispositiu de xarxa, com un equilibrador de càrrega) està manipulant la capçalera "Amfitrió" o que utilitzeu una eina o una biblioteca per emetre una sol·licitud manualment i definiu la capçalera "Amfitrió" incorrecta.
    Les sol·licituds han d\'incloure una capçalera "amfitrió" vàlida.',
      'Setup MySQL Schema' => 'Configureu l’esquema MySQL',
      'This option has been removed, you can use Dashboards to provide homepage customization. See T11533 for more details.' => 'Aquesta opció s\'ha eliminat, podeu utilitzar els quadres de Comandament per proporcionar personalització de la pàgina d\'inici. Vegeu T11533 per a més detalls.',
      ...
      'This key has the wrong uniqueness setting.' => 'Aquesta clau té una configuració de singularitat incorrecta.',
    ...
    Us recomanem que actualitzeu a MySQL 5.5 o una versió posterior.',
    ...
    El vostre servidor web està configurat incorrectament i grans parts de Phabricator no funcionaran fins que no es corregeixi aquest problema.
    ...
    Utilitzeu %s per iniciar dimonis. Consulteu %s per obtenir més informació.',
      'The minimum supported version of Subversion is 1.5, which was released in 2008.' => 'La versió suportada mínima de Subversion és 1.5, que va ser alliberada el 2008.',
    ...
    %s' => 'L\'amfitrió de la base de dades «%s» està utilitzant el fitxer de «paraules buides» incorporat per crear índexs de cerca. Això pot fer que la característica de cerca del Phabricator sigui menys útil. 
    ...
    Phabricator funcionarà amb l\'extensió anterior, però es recomana actualitzar a l\'extensió més recent.
    ...
    This option defines a list of netblocks which Phabricator will decline to connect to. Generally, you should list all private IP space here.' => 'Els usuaris de Phabricator poden fer sol·licituds a altres serveis de l\'amfitrió de Phabricator en algunes circumstàncies (per exemple, creant un repositori amb un URL remot o fent que Phabricator obtingui una imatge d\'un servidor remot). <br><br>Això pot representar una vulnerabilitat de seguretat si els serveis de la mateixa subxarxa accepten ordres o revelen informació privada sobre HTTP GET no autenticat, basada en l\'adreça IP de la font.  En particular, tots els amfitrions de la CE2 tenen accés a aquest servei. <br><br>Aquesta opció defineix una llista de blocs de xarxa als quals el Phabricator declinarà connectar-se.  En general, hauríeu de llistar tot l\'espai IP privat aquí.',
    ...
    També podeu augmentar el límit o ignorar aquest problema i acceptar que podeu trobar problemes per pujar fitxers grans i processar peticions grans.',
      ...
      'Format for the HTTP access log. Use `%s` to set the path. Available variables are:' => 'Format per al registre d’accés HTTP. Utilitzeu `%s` per establir el camí. Les variables disponibles són:',
    ...
    Encara que normalment és difícil per a un atacant convèncer Phabricator de connectar-se a un servidor MySQL maliciós, hauríeu de desactivar aquesta opció: aquesta capacitat és innecessària i inherentment perillosa. 
    Per desactivar aquesta opció, estableix: %s',
      ...
      'The path for local repositories does not exist, or is not readable by the webserver.' => 'El camí pels repositoris locals no existeix, o no és llegible pel servidor web.',
    ...
    If not set, no log will be written.' => 'Per habilitar el registre d\'accés de Phabricator, especifiqueu un camí d\'accés. L’accés a Phabricator que els registres d’accés HTTP normals (per exemple, pot mostrar usuaris, controladors i altres dades de l’aplicació).
    ...
    Requests must include a valid "Host" header.' => 'Aquesta sol·licitud no inclou cap capçalera "amfitrió". Això pot significar que el vostre servidor web (com ara nginx o apache) està mal configurat, de manera que la capçalera "Host" no es subministra a Phabricator o que feu una sol·licitud raw "Host" mitjançant una eina o una biblioteca.
    ...
    També és possible (però molt poc probable) que un altre element de xarxa (com ara un equilibrador de càrrega) estigui eliminant la capçalera.
    Les sol·licituds han d\'incloure una capçalera "amfitrió" vàlida.',
      ...
      'Unsafe MySQL "local_infile" Setting Enabled' => 'Configuració de MySQL \'local_infile\' no segura habilitada',
    ...
    Si no voleu instal·lar Pygments, podeu ignorar aquest problema.',
      'On host "%s", MySQL is configured with a small "%s" (%d), which may cause some large writes to fail. The recommended minimum value for this setting is "%d".' => 'A l\'amfitrió \'%s\', MySQL està configurat amb un petit \'%s\' (%s), cosa que pot provocar que fallin algunes escriptures grans. El valor mínim recomanat per a aquesta configuració és "%s".',
    ...
    (Si ja heu creat taules, MySQL va utilitzar incorrectament algun altre motor per crear-les. Cal convertir-les, deixar-les anar i reinicialitzar-les.)',
    ...
    Si esteu segur que teniu instal·lada una versió recent de \'%s\' i que funciona correctament, normalment és segur ignorar aquest advertiment.',
      ...
      'Config option \'%s\' is invalid. The URI must NOT have a path, e.g. \'%s\' is OK, but \'%s\' is not. Phabricator must be installed on an entire domain; it can not be installed on a path.' => 'L\'opció de configuració «%s» no és vàlida. L\'URI NO ha de tenir un camí, p. ex. \'%s\' està BÉ, però \'%s\' no. Phabricator ha d\'estar instal·lat en tot un domini; no es pot instal·lar en un camí.',
      ...
      'This will only affect the current web frontend. Daemons and any other web frontends may continue to use older, cached code from their opcache.' => 'Això només afectarà el web actual frontend. Daemons I qualsevol altre web frontends pot continuar utilitzar més vell, cached codi del seu opcache.',
    ...
    %s
    To configure the base URI, run the command shown below.' => 'L\'URI base per a aquesta instal·lació no està configurat, i les característiques principals no funcionaran correctament fins que la configureu. 
    Hauríeu d\'establir l\'URI base a l\'URI que utilitzareu per accedir al Phabricator, com "http://phabricator.example.com/". 
    ...
    Per configurar l\'URI base, executeu l\'ordre que es mostra a continuació.',
      ...
      'Report this Issue to the Upstream' => 'Notifiqueu aquest problema a l\'ascendent',
      'Repos' => 'Repositoris',
    ...
    Una vegada activeu DarkConsole per a la instal·lació, **cal habilitar-la per al vostre compte abans que aparegui a les pàgines.** Ho podeu fer a Configuració > Configuració del desenvolupador. 
    ...
    PHP va eliminar les funcions de manipulació de senyals que Phabricator requereix en PHP 7.0, i no les va restaurar fins a PHP 7.1. 
    Actualitza a PHP 7.1 o posterior (recomanat) o baixa a una versió més antiga de PHP 5 (descurat).',
    ...
    %s',
      'Database host "%s" is configured as a master, but is replicating another host. This is dangerous and can mangle or destroy data. Only replicas should be replicating. Stop replication on the host or reconfigure Phabricator.' => 'El servidor de base de dades «%s» està configurat com a mestre, però està replicant un altre amfitrió.  Això és perillós i pot manipular o destruir dades.  Només s\'haurien de reproduir rèpliques. Atura la replicació a l\'amfitrió o reconfigura Phabricator.',
    ...
    Configureu un CDN (o un domini de fitxer alternatiu) per eliminar aquest risc. L’ús d’un CDN també millorarà el rendiment. Consulteu la guia següent per obtenir instruccions.',
      ...
      'Missing Repository Local Path' => 'Falta el camí local del repositori',
    ...
    Disable this setting to continue.' => 'El vostre servidor està configurat amb "%s", cosa que impedeix que Phabricator obri fitxers als quals requereix accés. Desactiveu aquesta configuració per continuar.',
    ...
    Normalment, "http" i "https" han d\'estar presents en aquest conjunt.  Si elimines un o ambdós protocols, algunes característiques Phabricator que depenen d\'enllaços o redireccions poden no funcionar.',
    ...
    You may be able to install the native driver with a command like: %s' => 'PHP està utilitzant actualment el controlador extern MySQL més antic en lloc del controlador natiu MySQL més nou. El controlador més antic no té opcions i característiques (com ara el suport per als temps d\'espera de consulta) que permeten al Phabricator interactuar millor amb la base de dades. 
    Phabricator treballarà amb el conductor més antic, però es recomana actualitzar al conductor natiu. 
    ...
    %s' => 'Elasticsearch està configurat (amb el paràmetre %s) però Phabricator ha trobat una excepció en provar l\'índex. 
    %s',
      ...
      'HTTP Path Rewriting Incorrect' => 'La reescriptura del camí HTTP és incorrecta',
      'Surplus' => 'Sobrant',
      ...
      'PHP setting "%s" should be set to "-1" to avoid deprecation warnings.' => 'La configuració de PHP \'%s\' s\'ha d\'establir a \'-1\' per evitar avisos de deprecació.',
    );
  }
