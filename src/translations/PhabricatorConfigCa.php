...
Usually, "http" and "https" should be present in this set. If you remove one or both protocols, some features which rely on links or redirects may not work.' => 'Quan els usuaris escriuen comentaris que tenen URI, es convertiran automàticament en enllaços que es puguin clicar si el protocol URI apareix en aquest conjunt. 
...
Heu d\'activar DarkConsole establint «%s» abans que aquesta opció tingui cap efecte.',
  ...
  'This server is not configured in cluster mode.' => 'El Phabricator no està configurat en mode clúster.',
...
If you can not or do not want to enable "STRICT_ALL_TABLES", you can safely ignore this warning. This software will work correctly with this mode enabled or disabled.' => 'A l\'amfitrió de base de dades «%s», la configuració global «sql.mode» no inclou el mode "STRICT_ALL_TABLES". Es recomana activar aquest mode per millorar en general com el MySQL gestiona certs errors. 
...
This option is not compatible with this software. Remove \'%s\' from your configuration to continue.' => 'Teniu activat \'%s\' a la configuració de PHP.
...
Després d\'executar `bin/auth unlock` i fer els canvis a la configuració del proveïdor d\'autenticació, haureu d\'executar `bin/auth lock`.',
  ...
  'Daemons no longer use PID files.' => 'Els dimonis de Phabricator ja no utilitzen fitxers PID.',
...
Make sure databases connection information and MySQL are correctly configured.' => 'No s\'ha pogut connectar a MySQL! 
...
Assegureu-vos que Phabricator i MySQL estan configurats correctament.',
  ...
  'You can provide a custom highlighter engine by extending class %s.' => 'Phabricator pot ressaltar PHP per defecte i utilitzar Pygments per a altres idiomes si està habilitat. Podeu proporcionar un motor de ressaltador personalitzat ampliant la classe %s.',
...
To learn more about how this issue affects this software, see %s.' => 'La versió de %s d’aquest sistema està obsoleta i conté una important vulnerabilitat àmpliament coneguda (l’error "Shellshock").
...
Your webserver may not be configured to forward HTTP basic authentication. If you plan to use basic authentication (for example, to access repositories) you should reconfigure it.' => 'Phabricator es va enviar a si mateix una sol·licitud de prova amb una capçalera HTTP "Authorització", i esperava que aquestes credencials es transmetessin.  No obstant això, estaven absents o eren incorrectes quan eren rebuts.  Phabricator ha enviat el nom d\'usuari «%s» amb la contrasenya «%s»; ha rebut el nom d\'usuari «%s» i la contrasenya «%s». <br><br>És possible que el vostre servidor web no estigui configurat per reenviar l\'autenticació bàsica HTTP.  Si voleu utilitzar l\'autenticació bàsica (per exemple, per accedir als repositoris) heu de reconfigurar-la.',
  'Controls whether email is sent "From" users.' => 'Controla si el Phabricator envia un correu electrònic d\'usuaris "De".',
...
L\'usuari que executa el servidor web ha de poder llegir tots els directoris de PATH per fer-ne ús.',
  ...
  'You can rebuild repository identities while the server is running.' => 'Podeu reconstruir les identitats dels repositoris mentre el Phabricator s\'està executant.',
...
' => 'Personalitzeu la imatge i el text del logotip que apareixen a la capçalera del lloc principal:
  ...
  'This configuration option is unknown. It may be misspelled, or have existed in a previous version of the software.' => 'Aquesta opció de configuració és desconeguda. Pot estar mal escrita o haver existit en una versió anterior de Phabricator.',
...
(If you already created tables, MySQL incorrectly used some other engine to create them. You need to convert them or drop and reinitialize them.)' => 'El motor \'InnoDB\' no està disponible a MySQL (a l\'amfitrió \'%s\'). Activeu InnoDB a la configuració de MySQL.
...
Configureu un CDN (o un domini de fitxer alternatiu) per eliminar aquest risc. L’ús d’un CDN també millorarà el rendiment. Consulteu la guia següent per obtenir instruccions.',
  ...
  'To update these %d value(s), edit your PHP configuration file, located here:' => 'Per actualitzar aquests valors de %d, editeu el fitxer de configuració de PHP, que es troba aquí:',
  ...
  'This software sent itself a test request that was compressed with "Content-Encoding: gzip", but received different bytes than it sent.' => 'Phabricator ha enviat una sol·licitud de prova que s\'ha comprimit amb \'Content-Encoding: gzip\', però ha rebut bytes diferents dels que va enviar.',
  ...
  'The request body that was received began:' => 'El cos de la petició que Phabricator ha rebut en començar:',
...
DarkConsole exposa dades potencialment confidencials (com consultes, traces de piles i configuració) de manera que generalment no l\'heu d\'activar en producció.',
  ...
  'Transaction mail is now always sent with "Precedence: bulk" to improve deliverability.' => 'Ara Phabricator sempre envia correu de transaccions amb \'Precedència: massiva\' per millorar la capacitat de lliurament.',
  'Without \'%s\', this software can not test for the availability of other binaries.' => 'Sense \'%s\', Phabricator no pot provar la disponibilitat d\'altres binaris.',
...
If you configure clustering, note that this setting is ignored by intracluster requests.' => 'Si el servidor web respon tant a les peticions HTTP com HTTPS però voleu que els usuaris es connectin només amb HTTPS, podeu establir-ho a `cert` per a fer que el Phabricator redirigi les peticions HTTP a HTTPS. 
...
(Aquest problema pot ser causat per la falta d\'un \'QSA\' a la vostra RewriteRule.)',
  ...
  'Enable the debugging console.' => 'Activeu la consola de depuració de Phabricator.',
...
Alguna cosa està mal configurada o bé responen malament.',
  ...
  'The timezone this software should use by default.' => 'Hauria d’utilitzar-se el fus horari de Phabricator.',
  ...
  'To update these %d value(s), run these command(s) from the command line:' => 'Per actualitzar aquests %d valors, executeu aquestes ordres des de la línia d\'ordres:',
...
Per resoldre aquest problema, traieu tots els URI de \'feed.http-hooks\'.',
  ...
  'The current PHP configuration has these %d value(s):' => 'La configuració actual de PHP té aquests %d valors:',
  'Database host "%s" is configured as a master, but is replicating another host. This is dangerous and can mangle or destroy data. Only replicas should be replicating. Stop replication on the host or adjust configuration.' => 'L\'amfitrió de base de dades «%s» està configurat com a mestre, però un altre amfitrió està replicant. Això és perillós i pot manipular o destruir dades. Només s\'haurien de replicar les rèpliques. Atura la replicació a l\'amfitrió o reconfigura Phabricator.',
  ...
  'Done. Migrated %d keys.' => 'Fet. %d claus migrades.',
...
When cluster addresses are defined, hosts will also reject requests to interfaces which are not whitelisted.' => 'Definiu un clúster de Phabricator proporcionant una llista blanca d’adreces d’amfitrió que formen part del clúster.
...
Quan es defineixen les adreces de clúster, els amfitrions de Phabricator també rebutjaran les sol·licituds d’interfícies que no estan incloses a la llista blanca.',
  'On host "%s", MySQL is configured with a small "%s" (%d), which may cause some large writes to fail. The recommended minimum value for this setting is "%d".' => 'A l\'amfitrió \'%s\', MySQL està configurat amb un petit \'%s\' (%d), cosa que pot provocar que fallin algunes escriptures grans. El valor mínim recomanat per a aquest paràmetre és "%d".',
...
NOTA: Heu d\'instal·lar XHProf per a que funcioni l\'anàlisi de perfils.',
  ...
  'The environmental variable %s is empty. This server will not be able to execute some commands.' => 'La variable d\'entorn %s està buida. Phabricator no podrà executar algunes ordres.',
  ...
  'The current MySQL configuration has these %d value(s):' => 'La configuració actual de MySQL té aquests %d valors:',
...
Heu de configurar un domini de fitxers CDN o alternatiu per mitigar aquest risc. La configuració d’un CDN també millorarà el rendiment. Vegeu [[ %s | %s ]] per obtenir instruccions.',
  ...
  'The \'%s\' binary could not be found. Set the webserver\'s %s environmental variable to include the directory where it resides, or add that directory to \'%s\' in configuration.' => 'No s\'ha pogut trobar el binari \'%s\'. Establiu la variable d\'entorn de %s del servidor web per incloure el directori on resideix o afegiu-lo a \'%s\' a la configuració de Phabricator.',
  'MySQL is configured (on host "%s") to only index words with at least %d characters.' => 'MySQL està configurat (a l\'amfitrió \'%s\') per indexar només paraules amb almenys %d caràcters.',
  ...
  'Global customization of monospaced fonts is no longer supported.' => 'Phabricator ja no és compatible amb la personalització global de les fonts monoespaiades.',
  ...
  'This server received an "X-Mod-Pagespeed" or "X-Page-Speed" HTTP header on this request, which indicates that you have enabled "mod_pagespeed" on this server. This module is not compatible with this software. You should disable the module.' => 'Phabricator ha rebut una capçalera HTTP "X-Mod-Pagespeed" o "X-Page-Speed" en aquesta sol·licitud, que indica que heu activat "mod_pagespeed" en aquest servidor. Aquest mòdul no és compatible amb Phabricator. Haureu de desactivar-lo.',
  ...
  'Allow a single mailbox to be used for all replies.' => 'Permet que Phabricator utilitzi una sola bústia de correu per a totes les respostes.',
...
Switch the service to read-only mode. In this mode, users will be unable to write new data. Normally, the cluster degrades into this mode automatically when it detects that the database master is unreachable, but you can activate it manually in order to perform maintenance or test configuration.' => 'ADVERTÈNCIA: Aquesta és una opció prototip i la descripció següent és actualment pura fantasia.
...
Canvieu Phabricator al mode de només lectura. En aquest mode, els usuaris no podran escriure dades noves. Normalment, el clúster es degrada a aquest mode automàticament quan detecta que no es pot accedir al mestre de base de dades, però podeu activar-lo manualment per realitzar el manteniment o la configuració de proves.',
  ...
  '%d related link(s):' => '%d enllaços relacionats:',
...
Les sol·licituds han d\'incloure una capçalera "amfitrió" vàlida.',
  ...
  'Sets the default color scheme.' => 'Estableix l\'esquema de colors per defecte de Phabricator.',
  'Clustering Introduction' => 'Introducció al clúster',
  'By default, this software includes some flavor text in the UI, like a prompt to "Weigh In" rather than "Add Comment" in Maniphest. If you\'d prefer more traditional UI strings like "Add Comment", you can set this flag to disable most of the extra flavor.' => 'Per defecte, Phabricator inclou algunes variants de text a la interfície gràfica d’usuari, com un indicador de "Fes bon pes" en lloc d\'"Afegir un comentari" a Maniphest... Si preferiu cadenes UI més tradicionals com ara "Afegir un comentari", podeu configurar aquest indicador de seguiment per desactivar la majoria de les variacions addicionals.',
...
%s',
  ...
  'You can usually install a PHP extension using %s, %s, or %s. A common package name is %s. Try commands like these:' => 'Normalment podeu instal·lar una extensió PHP amb %s o %s. Els noms habituals dels paquets són de %s o %s. Proveu ordres com aquestes:',
...
Si proporcioneu un identificador d\'instància aquí (normalment injectant-lo amb un "%s"), Phabricator el passarà a subprocessos i hooks de validació a la variable d\'entorn "%s".',
  ...
  'Arbitrarily large files are now supported. Consult the documentation for configuration details.' => 'Ara Phabricator admet fitxers de mida arbitrària. Consulteu la documentació per obtenir informació detallada sobre la configuració.',
  ...
  'Without \'%s\', this software will not be able to generate or render diffs in multiple applications.' => 'Sense \'%s\', Phabricator no podrà generar ni renderitzar diferències en diverses aplicacions.',
