...
`policy`에 대한 값은 `action`(`%s` 또는 `%s`), `rule`(PolicyRule 클래스), 그리고 `value` 배열을 포함하는 배열이어야 합니다.',
  ...
  'Enable expert mode to reveal additional "advanced" options in the Phabricator UI.' => '전문가 모드를 활성화하면 파브리케리터 사용자 인터페이스에 추가적인 "고급" 옵션이 표시됩니다.',
...
다음 단계에서는파브리케이터 OAuth에서 사용할 미디어위키 OAuth 컨슈머를 생성합니다.',
  ...
  'Create custom access control policy objects.' => '사용자 지정 접근 제어 정책 객체를 생성합니다.',
