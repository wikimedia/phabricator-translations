...
final class PhabricatorSettingsZhHans
  ...
  protected function getTranslations() {
    ...
    包含在邮件正文中，以便基于正文内容的邮件规则可以路由邮件。',
      'Your browser timezone (%s) differs from your profile timezone (%s). You can adjust your profile setting to match your browser, or ignore this conflict to keep your current profile setting.' => '您的浏览器时区（%s）与您的个人资料时区（%s）不同。您可以调整您的个人资料设置以匹配您的浏览器时区，或者忽略此冲突以保留您当前的个人资料设置。',
    ...
    这些参数设置**只**应用到您连接的对象（例如，你是评审人的修订或 CC 您的任务））。要在其他对象创建时接收电子邮件通知，请配置[[ /herald/ | 传令规则 ]]。',
      ...
      'Add Authentication Factor' => '添加身份验证因子',
    ...
    请参阅下表了解受支持的变量和协议。',
      ...
      'Revoke All' => '吊销所有',
      'No settings panel group with key "%s" exists!' => '不存在键为“%s”的设置面板组！',
