...
final class PhabricatorTypeaheadCa
  ...
  protected function getTranslations() {
    return array(
      'Typeahead Results' => 'Completa els Resultats Semiautomàticament',
    ...
    %s' => 'A més de teclejar noms d\'usuari i de projecte per construir consultes, també podeu escriure els noms de funcions especials que us donen més opcions i la capacitat d\'expressar consultes més complexes. 
    Les funcions tenen un nom intern (com `%s`) i un nom llegible per humans, com `Current Viewer`.  En general, es pot escriure un per seleccionar la funció.  També podeu fer clic al botó {nav icon=search} en qualsevol control de capçalera per explorar les funcions disponibles i trobar aquesta documentació. 
    ...
    Note that different controls support //different// functions (depending on what the control is doing), so these specific functions may not work everywhere. You can always check the help for a control to review which functions are available for that control.' => 'Les funcions semiautomàtiques són una característica avançada que permet construir consultes més poderoses.  Aquest document explica les funcions disponibles per al control seleccionat. <br><br>Per obtenir ajuda general amb la cerca, vegeu la [[ %s | Guia d\'usuari de cerca ]] a la documentació. <br><br>Tingueu en compte que els diferents controls suporten //diferents// funcions (depenent del que estigui fent el control), de manera que aquestes funcions específiques poden no funcionar a tot arreu.  Sempre podeu comprovar l\'ajuda d\'un control per revisar quines funcions estan disponibles per a aquest control.',
      ...
      'Unable to parse function and arguments for token "%s".' => 'No s\'ha pogut analitzar la funció i els arguments del testimoni «%s».',
      ...
      'Typeahead Function Help' => 'Ajuda sobre la funció de completar semiautomàticament',
      'Sprite' => 'Sprite',
      ...
      'Auto' => 'Automàtic',
      'You reach the edge of the abyss.' => 'Assoleixes la vora de l\'abisme.',
      'Tokenization of "%s"' => 'Lexicalització de "%s"',
      'Browse Numbers' => 'Navega pels números',
      'Priority Type' => 'Tipus de Prioritat',
      'Using Typeahead Functions' => 'Ús de funcions per a completar semiautomàticament',
    );
  }
