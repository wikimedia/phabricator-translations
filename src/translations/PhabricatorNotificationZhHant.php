...
final class PhabricatorNotificationZhHant
  ...
  'Notification server configuration describes an invalid host ("%s", at index "%s") with an unrecognized type ("%s"). Valid types are "%s" or "%s".' => '通知伺服器設置以無法識別的類型（「%3$s」）描述出一台無效的主機（「%s」，位在索引「%s」）。有效的類型為「%4$s」或「%5$s」。',
  'Notification server configuration describes an invalid host ("%s", at index "%s") with an invalid protocol ("%s"). Valid protocols are "%s" or "%s".' => '通知伺服器設置以無效的埠號（「%3$s」）描述出一台無效的主機（「%s」，位在索引「%s」）。有效的埠號為「%4$s」或「%5$s」。',
