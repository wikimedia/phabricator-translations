...
final class PhabricatorNotificationTr
  ...
  'Notification server configuration describes an invalid host ("%s", at index "%s") with an unrecognized type ("%s"). Valid types are "%s" or "%s".' => 'Bildirim sunucusu yapılandırması, tanınmayan türde ("%3$s") geçersiz bir ana bilgisayarı ("%2$s" dizininde "%s") tanımlar. Geçerli türler "%4$s" veya "%5$s"\'dir.',
  'Notification server configuration describes an invalid host ("%s", at index "%s") with an invalid protocol ("%s"). Valid protocols are "%s" or "%s".' => 'Bildirim sunucusu yapılandırması geçersiz bir protokolle ("%3$s") geçersiz bir ana bilgisayarı ("%2$s" dizininde "%s") tanımlar. Geçerli protokoller "%4$s" veya "%5$s" şeklindedir.',
...
%s' => 'Bu sunucu, bir bildirim sunucusu kullanacak şekilde yapılandırılmış, ancak bağlanamıyor. Bu sorunu çözmeli veya bildirim sunucusunu devre dışı bırakmalısınız. Yapılandırmanızı iki kez kontrol etmeniz veya aşağıdaki komutu kullanarak sunucuyu yeniden başlatmanız yararlı olabilir.

%s',
  'Unable to test client on an admin server!' => 'İstemci bir yönetici sunucusunda test edilemiyor!',
  'Notification server configuration describes an invalid host ("%s", at index "%s"). This is an "admin" service but it has a "path" property. This property is only valid for "client" services.' => 'Bildirim sunucusu yapılandırması geçersiz bir ana bilgisayarı ("%2$s" dizininde "%s") açıklar. Bu bir "yönetici" servisidir ancak "yol" özelliğine sahiptir. Bu özellik yalnızca "müşteri" hizmetleri için geçerlidir.',
...
%s',
  ...
  'This server is configured to use a notification server, but is not able to connect to it.' => 'Bu sunucu bir bildirim sunucusu kullanacak şekilde yapılandırılmış ancak ona bağlanamıyor.',
