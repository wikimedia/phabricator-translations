...
final class PhabricatorConduitFrFR
  ...
  'Reading input from stdin...' => 'Lecture des entrées depuis le flux d’entrée standard...',
  'All Call Logs' => 'Tous les journaux d’appels',
  'File to read parameters from, or "-" to read from stdin.' => 'Fichier depuis lequel lire les paramètres, ou bien « - » pour les lire depuis le flux d’entrée standard.',
...
Vérifiez votre syntaxe. La valeur était : %s.',
  ...
  'Caller' => 'Appelant',
  'This request originates from outside of the cluster address range. Requests signed with cluster API tokens must originate from within the cluster.' => 'Cette demande provient de l\'extérieur de la plage d\'adresses du cluster. Les requêtes signées avec des jetons d\'API de cluster doivent provenir du cluster.',
  ...
  'Your \'%s\' client version is \'%d\', which is newer than the server version, \'%d\'. Upgrade your server.' => 'La version de votre client « %s » est « %d », qui est plus récente que la version du serveur, « %d ». Mettez à niveau votre serveur.',
  ...
  'The key which signed this request is associated with a disabled device ("%s").' => 'La clé qui a signé cette requête est associée à un dispositif désactivé (« %s »).',
  ...
  'A new version of arc is available! You need to upgrade to connect to this server (you are running version %d, the server is running version %d).' => 'Une nouvelle version d’arc est disponible ! Vous devez vous mettre à jour pour vous connecter à ce serveur (vous exécutez la version %d, le serveur la version %d).',
  ...
  'This token is automatically generated, and used to make requests between nodes in a cluster. You can not use this token in external applications.' => 'Ce jeton est automatiquement généré et utilisé pour effectuer des requêtes entre les nœuds d\'un cluster. Vous ne pouvez pas utiliser ce jeton dans des applications externes.',
  ...
  'This request originates from outside of the cluster address range. Requests signed with trusted device keys must originate from within the cluster.' => 'Cette demande provient de l\'extérieur de la plage d\'adresses du cluster. Les demandes signées avec des clés de périphérique approuvées doivent provenir du cluster.',
  ...
  'Expected a nonempty list, but value is an empty list.' => 'Une liste non vide était attendue, mais la valeur est une liste vide.',
  ...
  'Epoch timestamp must be larger than 0, got %d.' => 'L’horodatage Epoch doit être plus grand que 0, %d obtenu.',
  ...
  'Show old methods which will be deleted in a future version of this software.' => 'Afficher les anciennes méthodes qui seront supprimées dans une future version de ce logiciel.',
);
  }
