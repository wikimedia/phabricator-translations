...
final class PhabricatorSystemCa
  ...
  protected function getTranslations() {
    return array(
      'An Administrator has placed this server into read-only mode.' => 'Un administrador ha situat Phabricator en mode de només lectura.',
      ...
      'Destroying %s **%s**...
    ' => 'Destruint %s **%s**...',
      'This server has consistently been unable to reach the writable ("master") database while processing recent requests.' => 'Phabricator ha estat constantment incapaç d’arribar a la base de dades d’escriptura ("mestra") mentre processava les sol·licituds recents.',
      ...
      'This server was unable to connect to the writable ("master") database while handling this request, and automatically degraded into read-only mode.' => 'Phabricator no ha pogut connectar-se a la base de dades que es podia escriure ("mestre") mentre gestionava aquesta sol·licitud i es va degradar automàticament al mode de només lectura.',
    ...
    Aquests flancs trencats són una conseqüència esperada de la destrucció d\'objectes, i el ascendent de Phabricator no t\'ajudarà a corregir-los. Recomanem encaridament desactivar o arxivar els objectes al seu lloc.',
      ...
      'View Affected Path Index' => 'Mostra l\'índex del camí afectat',
      ...
      'Read-only mode was enabled by the explicit action of a human administrator, so you can get more information about why it has been turned on by rolling your chair away from your desk and yelling "Hey! Why is %s in read-only mode??!" using your very loudest outside voice.' => 'El mode de només lectura es va habilitar amb l\'acció explícita d\'un administrador humà, de manera que podeu obtenir més informació sobre per què s\'ha activat estirant la cadira de l\'escriptori i cridant "Ei! Per què Phabricator està en mode de només lectura? ?! " utilitzant la vostra veu exterior més forta.',
      ...
      'This may happen if there is a temporary network anomaly on the server side, like cosmic radiation or spooky ghosts. If this failure was caused by a transient service interruption, this server will recover momentarily.' => 'Això pot passar si hi ha una anomalia temporal de xarxa al costat del servidor, com ara la radiació còsmica o els fantasmes fantasmagòrics. Si aquest error es va produir per una interrupció transitòria del servei, Phabricator es recuperarà momentàniament.',
