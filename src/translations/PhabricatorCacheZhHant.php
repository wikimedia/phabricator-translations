...
final class PhabricatorCacheZhHant
  ...
  protected function getTranslations() {
    return array(
      ...
      'Select caches to purge with "--all" or "--caches". Available caches are: %s.' => '以「--all」或「--caches」來選擇快取進行清除。可用的快取為：%s。',
      ...
      'Enabling APC/APCu will improve performance.' => '啟用 APC/APCu 將會增進效能。',
      'Zend OPcache Not Installed' => '未安裝 Zend OPcache',
      '"%s" Enabled' => '「%s」已啟用',
      'Markup Cache' => '標記快取',
      'The PHP "Zend OPcache" extension is installed, but not enabled in your PHP configuration. Enabling it will dramatically improve Phabricator performance. Edit the "%s" setting to enable the extension.' => 'PHP 的「Zend OPcache」擴充套件已安裝，但未在您的 PHP 設置上啟用。啟用它將會顯著地提升 Phabricator 效能。編輯「%s」設定可啟用此擴充套件。',
      'General Cache' => '一般快取',
      ...
      'Installing the "APCu" PHP extension will improve performance. This extension is strongly recommended. Without it, Phabricator must rely on a very inefficient disk-based cache.' => '安裝「APCu」PHP 擴充套件將會增進效能。此擴充套件被強烈建議使用。若不使用的話，Phabricator 就只能依靠效率低的磁碟快取。',
      ...
      'The APC or APCu PHP extensions are installed, but not enabled in your PHP configuration. Enabling these extensions will improve Phabricator performance. Edit the "%s" setting to enable these extensions.' => 'APC 或 APCu PHP 擴充套件已安裝，但未在您的 PHP 設置上啟用。啟用這些將會提升 Phabricator 效能。編輯「%s」設定可啟用這些擴充套件。',
      ...
      'Enabling OPcache will dramatically improve performance.' => '啟用 OPcache 將會顯著地增進效能。',
