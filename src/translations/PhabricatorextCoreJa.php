...
final class PhabricatorextCoreJa
  ...
  protected function getTranslations() {
    return array(
      'Escalate' => 'エスカレート',
      'Next: %s' => '次は: %s',
      'Provides instructions for applying a local patch using just git.' => 'gitだけでローカルパッチを適用する方法を説明しています。',
    ...
    NOTE: This feature is provided by a Wikimedia-maintained extension, ProjectBurnupGraphProfileMenuItem. See {rPHEX} for the source.' => 'これにより、このプロジェクトでタグ付けされたオープンタスクを maniphest で検索するためのリンクが追加されます。
    注: この機能は、Wikimedia-maintained が管理する拡張機能 ProjectBurnupGraphProfileMenuItem によって提供されています。出典は{rPHEX}を参照してください。',
      ...
      'This adds a link to search maniphest for open tasks which are tagged with this project.
    NOTE: This feature is provided by a Wikimedia-maintained extension, ProjectOpenTasksProfileMenuItemextension. See {rPHEX} for the source.' => 'このプロジェクトでタグ付けされたオープンタスクを検索するマニフェストへのリンクが追加されます。
    注：この機能はWikimedia-maintainedが管理する拡張機能、ProjectOpenTasksProfileMenuItemextensionによって提供されます。ソースは {rPHEX} をご覧ください。',
      ...
      'The requested project does not exist' => '要求されたプロジェクトは存在しません',
      'Burnup Graph' => '燃焼グラフ',
      ...
      'Deny' => '拒否',
      'Consumer Key is required' => 'Consumer Keyが必要です。',
      'Invalid' => '無効',
      'Change type to' => '以下のタイプに変更',
      'Required' => '必須',
      'Download tar.gz' => 'tar.gz をダウンロード',
    );
  }
