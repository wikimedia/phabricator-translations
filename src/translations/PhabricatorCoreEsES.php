<?php

final class PhabricatorCoreEsES
  extends PhutilTranslation {

  public function getLocaleCode() {
    return 'es_ES';
  }

  protected function getTranslations() {
    return array(
  'Question' => 'Pregunta',
  'No repository "%s" exists!' => 'No existe el repositorio «%s».',
  '%s edited %s.' => '%s editó %s.',
  'Move Document' => 'Mover documento',
  'Maximum must be larger than minimum.' => 'El máximo tiene que ser más grande que mínimo.',
  'User "%s" is not a valid user.' => 'El usuario "%s" no es válido.',
  'Authored' => 'Creado por',
  'Creators' => 'Creadores',
  'Minus' => 'Menos',
  'OPTIMIZE' => 'OPTIMIZAR',
  'Raised Concern' => 'Preocupación comunicada',
  'Create Document' => 'Crear documento',
  'Format' => 'Formato',
  'Commented On' => 'Comentó en',
  '%s added %s task(s): %s.' => '%s añadió %s tarea(s): %s.',
  'Reopened' => 'Reabierto',
  'Untitled Document' => 'Documento sin título',
  'Executed %s task(s).' => 'Tarea(s) %s ejecutada(s).',
  'No visible events.' => 'No hay eventos visibles.',
  'MFA Authenticated' => 'Autenticado con MFA',
  'Browse Documents' => 'Examinar documentos',
  'Favorites' => 'Favoritos',
  'italic text' => 'texto en cursiva',
  'Custom' => 'Personalizado',
  '%s (Preview)' => '%s (previsualización)',
  'Tasks' => 'Tareas',
  'Repository URI' => 'URI del repositorio',
  'Inactive' => 'Inactivo',
  'View Email Body' => 'Ver cuerpo del email',
  'Email Addresses' => 'Direcciones de correo electrónico',
  'Email Address' => 'Dirección de correo electrónico',
  'Restoring "%s" to the default retention policy.' => 'Restaurando «%s» a la directiva de retención predeterminada.',
  'Orange' => 'Naranja',
  'Failed' => 'Falló',
  'Upload Picture' => 'Cargar imagen',
  'Packages' => 'Paquetes',
  'Assigned to Me' => 'Asignado a mí',
  'Author: %s' => 'Autor: %s',
  'Revoke' => 'Revocar',
  'Administrators' => 'Administradores',
  'Example' => 'Ejemplo',
  '%s confirmed this job.' => '%s confirmó este trabajo.',
  'Previous Day' => 'Día anterior',
  'MODIFIED' => 'MODIFICADO',
  '(Use Default)' => '(Utilizar predeterminado)',
  'Main Request' => 'Solicitud principal',
  'Unknown Error' => 'Error desconocido',
  '%s added %s task(s) to %s: %s.' => '%s añadió %s tarea(s) a %s: %s.',
  'Current Tasks' => 'Tareas actuales',
  'Summary' => 'Resumen',
  'Duration' => 'Duración',
  'Meme' => 'Meme',
  '(%s -> %s @ %s) %s' => '(%s → %s @ %s) %s',
  'Comma-Separated Values (.csv)' => 'Valores separados por comas (.csv)',
  'Paths' => 'Rutas',
  'Custom Fields' => 'Campos personalizados',
  'Comment' => 'Comentario',
  'Create a Task' => 'Crear una tarea',
  'Failure' => 'Fallo',
  'Malformed header.' => 'Cabecera con formato incorrecto.',
  'Group' => 'Grupo',
  'Other Project...' => 'Otro proyecto…',
  'Utilities' => 'Utilidades',
  'Tomorrow' => 'Mañana',
  'Variable' => 'Variable',
  'Bold' => 'Negrita',
  'Analyzing tables...' => 'Analizando tablas…',
  'Active Repositories' => 'Repositorios activos',
  'September' => 'Septiembre',
  'Grey' => 'Gris',
  'Color' => 'Color',
  'Order' => 'Orden',
  'Protocol' => 'Protocolo',
  'Administrator' => 'Administrador',
  'Complete' => 'Completado',
  'Open Status' => 'Estado abierto',
  'Visible' => 'Visible',
  'Exception' => 'Excepción',
  'Enable' => 'Activar',
  'Active Revisions' => 'Revisiones activas',
  'Up' => 'Subir',
  'Direct Subtask' => 'Subtarea directa',
  'This server is in read-only mode.' => 'Este servidor está en modo de solo lectura.',
  '(Show All)' => '(Mostrar todo)',
  'EXTRACT' => 'EXTRAER',
  'Drafts' => 'Borradores',
  'Wed' => 'Mié',
  'Draft' => 'Borrador',
  'Repository Identity' => 'Identidad del repositorio',
  'Today' => 'Hoy',
  'Mon' => 'Lun',
  'Current Viewer' => 'Vista actual',
  'Current Algorithm' => 'Algoritmo actual',
  '%s %s' => '%s %s',
  'Bug' => 'Error',
  'No notifications.' => 'Ninguna notificación.',
  'Object' => 'Objeto',
  'Show Only Bots' => 'Mostrar solo bots',
  'File' => 'Archivo',
  'Badges' => 'Insignias',
  'Note' => 'Nota',
  'Submit' => 'Enviar',
  'Continue' => 'Continuar',
  'Callsign' => 'Identificador',
  'Quote' => 'Citar',
  'Repository' => 'Repositorio',
  'Empty' => 'Vacío',
  'Any' => 'Cualquiera',
  'Service' => 'Servicio',
  'Edit Menu' => 'Editar menú',
  'Waiting' => 'Esperando',
  '%s' => '%s',
  'In Use' => 'En uso',
  '%s updated the description for %s.' => '%s actualizó la descripción de %s.',
  'Thu' => 'Jue',
  'Welcome to %s' => 'Te damos la bienvenida a %s',
  'Learn More' => 'Más información',
  '· · ·' => '· · ·',
  'Allow' => 'Permitir',
  'Broken' => 'Roto',
  'Creation (Newest First)' => 'Creación (Lo más nuevo primero)',
  'Browse Packages' => 'Explora paquetes',
  'November' => 'Noviembre',
  'Executing %s...' => 'Ejecutando %s...',
  'From' => 'De',
  'Edit Package' => 'Editar paquete',
  'Reset Password' => 'Restablecer contraseña',
  'Personal' => 'Personal',
  'NO TASKS' => 'NINGUNA TAREA',
  'Epoch timestamp when the object was created.' => 'Cronomarcador de época de cuando se creó el objeto.',
  'Class' => 'Clase',
  'Tuesday' => 'Martes',
  'SKIP' => 'SALTA',
  'March' => 'Marzo',
  '%s, %s' => '%s, %s',
  'Normal' => 'Normal',
  '%s was reprioritized (from "%d" to "%d").' => '%s fue repriorizado (del "%d" al "%d).',
  'New Password' => 'Contraseña nueva',
  'Sun' => 'Dom',
  'Change Password' => 'Cambiar contraseña',
  'UNARCHIVING' => 'DESARCHIVANDO',
  'Login' => 'Iniciar sesión',
  'Log Out' => 'Cerrar sesión',
  'Owners' => 'Propietarios',
  'Memory Usage: %s KB' => 'Uso de memoria: %s KB',
  'Current' => 'Actual',
  'Closed, Resolved' => 'Cerrado, Resuelto',
  'Image' => 'Imagen',
  'Path' => 'Ruta',
  'Stopped' => 'Parado',
  'Tue' => 'Mar',
  'Edit' => 'Editar',
  'Create Subtask' => 'Crear subtarea',
  'Invited' => 'Invitado',
  'Yes' => 'Sí',
  'Projects' => 'Proyectos',
  'Database Not Initialized' => 'No se inicializó la base de datos',
  'Pink' => 'Rosa',
  'GitHub' => 'GitHub',
  'Default Picture' => 'Imagen por defecto',
  'Invalid' => 'No válido',
  'Read/Write' => 'Leer/Escribir',
  'CANCELLED' => 'CANCELADO',
  'Fullscreen Mode' => 'Modo de pantalla completa',
  'Description' => 'Descripción',
  'Wednesday' => 'Miércoles',
  'Hide Bots' => 'Ocultar robots',
  'Updated %s' => 'Actualizado %s',
  'Users' => 'Usuarios',
  'Git' => 'Git',
  'Public Key' => 'Clave pública',
  'Design' => 'Diseño',
  'DOCUMENT DETAIL' => 'DETALLE DEL DOCUMENTO',
  'Assign task to' => 'Asignar tarea a',
  'TOTAL' => 'TOTAL',
  'PRIORITY' => 'PRIORIDAD',
  'Display Options' => 'Opciones de visualización',
  'Provide MFA Credentials' => 'Proporcionar credenciales de MFA',
  'Global' => 'Global',
  'Seconds' => 'Segundos',
  'Author' => 'Autor',
  'See Documentation' => 'Ver documentación',
  'Exact Names' => 'Nombres exactos',
  'Accepted' => 'Aceptado',
  'Target' => 'Destino',
  'Good' => 'Bueno',
  'Analysis' => 'Análisis',
  'This query is constrained by a project you do not have permission to see.' => 'Esta consulta está restringida por un proyecto que no tiene permiso para ver.',
  'New' => 'Nuevo',
  'Change' => 'Cambiar',
  'December' => 'Diciembre',
  'Extensions' => 'Extensiones',
  'Column' => 'Columna',
  'Partial' => 'Parcial',
  'Documents' => 'Documentos',
  'Message' => 'Mensaje',
  'Edit Related Tasks' => 'Editar tareas relacionadas',
  'August' => 'Agosto',
  'Queued' => 'En espera',
  'All' => 'Todos',
  'Aliases' => 'Alias',
  '%s changed the status of %s from %s to %s.' => '%s cambió el estado de %s de %s a %s.',
  'Sat' => 'Sáb',
  'Files' => 'Archivos',
  'Assigned: %s' => 'Asignado: %s',
  'Subscribers' => 'Suscriptores',
  'Name' => 'Nombre',
  'Restarting' => 'Reiniciando',
  'Notifications' => 'Notificaciones',
  'Thursday' => 'Jueves',
  'Title' => 'Título',
  'Applications' => 'Aplicaciones',
  'Real Name' => 'Nombre real',
  'Cancel' => 'Cancelar',
  'EXECUTE' => 'EJECUTAR',
  'Occurs Before' => 'Ocurre antes',
  'You must choose a project.' => 'Debes seleccionar un proyecto.',
  'Available' => 'Disponible',
  'Developer Settings' => 'Configuración de desarrollador',
  'Welcome to %s!' => '¡Le damos la bienvenida a %s!',
  'Error' => 'Error',
  'Retitled' => 'Retitulado',
  'No' => 'No',
  'Email' => 'Correo electrónico',
  'Username' => 'Nombre de usuario',
  '%s was delayed until "%s".' => '%s fue demorado hasta "%s".',
  'Account' => 'Cuenta',
  'Assigned Tasks' => 'Tareas asignadas',
  'October' => 'Octubre',
  'Language' => 'Idioma',
  'Recent Activity' => 'Actividad reciente',
  'Password' => 'Contraseña',
  'Temporary Tokens' => 'Tokens temporales',
  'Change Status' => 'Cambiar estado',
  'No data available.' => 'No hay datos disponibles.',
  'Monday' => 'Lunes',
  'No tasks selected to delay.' => 'Ningúna tarea seleccionada para demorar.',
  'Action' => 'Acción',
  '%s renamed %s from %s to %s.' => '%s renombró %s de %s a %s.',
  'project' => 'proyecto',
  'Creation (Oldest First)' => 'Creación (lo más antiguo primero)',
  'Documentation' => 'Documentación',
  'User' => 'Usuario',
  'Your session is in high security mode. When you finish using it, click here to leave.' => 'Su sesión está en modo de alta seguridad. Cuando termine de usarla, haga clic aquí para salir.',
  'Closed, Invalid' => 'Cerrado, Inválido',
  '%s updated the description.' => '%s actualizó la descripción.',
  'List of project hashtags, as array.' => 'Lista de hashtags del proyecto, como array.',
  'June' => 'Junio',
  'Created' => 'Creado',
  'Event' => 'Evento',
  'Calendar' => 'Calendario',
  '%s updated this object.' => '%s actualizó este objeto.',
  'Show All' => 'Mostrar todo',
  '%s has invited you to join %s.' => '%s te ha invitado a que te unas a Phabricator.',
  'Fri' => 'Vie',
  '%s created %s.' => '%s creó %s.',
  'Search' => 'Buscar',
  'Change priority to' => 'Cambiar prioridad a',
  'Text' => 'Texto',
  'Actions' => 'Acciones',
  'No Projects' => 'No hay proyectos',
  'Type a username or function...' => 'Escriba un nombre de usuario o función...',
  'Help' => 'Ayuda',
  'Find Project:' => 'Buscar proyecto:',
  'Settings' => 'Ajustes',
  'Repositories' => 'Repositorios',
  'Cancelled %s task(s).' => '%s tarea(s) cancelada(s).',
  'Assigned To' => 'Asignado a',
  'Authored By' => 'Escrito por',
  'Occurs After' => 'Ocurre después',
  'Name Contains' => 'Nombre Contiene',
  'Resolved' => 'Resuelto',
  'Current Picture' => 'Imagen actual',
  'Yesterday' => 'Ayer',
  'Languages' => 'Idiomas',
  'Relevance' => 'Relevancia',
  'July' => 'Julio',
  'View All Results' => 'Ver todos los resultados',
  'Sessions' => 'Sesiones',
  'Create a Project' => 'Crear un proyecto',
  'SSH Public Keys' => 'Claves públicas SSH',
  'Date' => 'Fecha',
  'Loading preview...' => 'Cargando vista previa...',
  'Archived' => 'Archivado',
  'Authentication' => 'Autenticación',
  'Comma-separated list of project hashtags.' => 'Lista de hashtags de proyectos separados por comas.',
  'Required' => 'Obligatorio',
  'Javelin Linter' => 'Optimizador Javelin',
  'Use Picture' => 'Usar imagen',
  'Disabled' => 'Desactivado',
  'Project' => 'Proyecto',
  'Save Changes' => 'Guardar cambios',
  'View Edit History' => 'Ver historial de ediciones',
  'Activity Logs' => 'Registro de actividad',
  'Log In or Register' => 'Iniciar sesión o registrarse',
  'Unassign task.' => 'Tareas sin asignar',
  'Best Available Algorithm' => 'Mejor algoritmo disponible',
  'WARNING' => 'ADVERTENCIA',
  'Wait Patiently' => 'Espera pacientemente',
  'Profile' => 'Perfil',
  'Real name is required.' => 'Se requiere un nombre real.',
);
  }

}
