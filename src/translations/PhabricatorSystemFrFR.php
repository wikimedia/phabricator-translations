...
final class PhabricatorSystemFrFR
  ...
  protected function getTranslations() {
    return array(
  'An Administrator has placed this server into read-only mode.' => 'Un administrateur a placé ce serveur en mode lecture seule.',
  ...
  'Panic!' => 'Panique !',
  'View Handle' => 'Voir le descripteur',
  'No Writable Database' => 'Aucune base de données disponible en écriture',
  'Destroying %s **%s**...
' => 'Destruction de %s **%s**...',
  'This server has consistently been unable to reach the writable ("master") database while processing recent requests.' => 'Ce serveur n\'a jamais réussi à accéder à la base de données principale théoriquement accessible en écriture lors du traitement des demandes récentes.',
  ...
  'This server was unable to connect to the writable ("master") database while handling this request, and automatically degraded into read-only mode.' => 'Ce serveur n’a pas pu se connecter à la base de données principale théoriquement accessible en écriture lors du traitement de cette requête et a été automatiquement dégradé en mode lecture seule.',
  ...
  'This request was served from a replica database. Replica databases may lag behind the master, so very recent activity may not be reflected in the UI. This data will be restored if the master database is restored, but may have been lost if the master database has been reduced to a pile of ash.' => 'Cette requête a été traitée à partir d\'une base de données répliquée. Les bases de données répliquées peuvent avoir du retard par rapport à la base principale, aussi des activités récentes peuvent ne pas apparaitre dans l’interface utilisateur. Ces données seront restaurées si la base de données principale est restaurée mais pourraient être perdues si la base principale est détruite.',
...
These tattered edges are an expected consequence of destroying objects, and the upstream will not help you fix them. We strongly recommend disabling or archiving objects instead.' => 'La destruction d’objets peut causer l’arrêt du fonctionnement des objets liés et peut laisser un peu partout des références vers ces objets qui n’existent plus. Dans la plupart des cas, il est préférable de désactiver ou d’archiver les objets au lieu de les détruire. Ce risque est accru lors de la suppression d’objets complexes ou fortement connectés tels que les dépôts de fichiers, les projets et les utilisateurs.

Ces à-côtés dégradés sont une conséquence prévisible de la destruction d’objets et le serveur en amont ne pourra pas vous aider à les corriger. Nous recommandons fortement de désactiver ou d\'archiver les objets à la place.',
  'IMPORTANT' => 'IMPORTANT',
  ...
  'This server will periodically retry the connection and recover once service is restored. Most causes of persistent service interruption will require administrative intervention in order to restore service.' => 'Ce serveur réessayera périodiquement la connexion et reprendra une fois le service rétabli. La plupart des causes d’interruption persistante de service nécessitent une intervention de l\'équipe de maintenance pour rétablir le service.',
  ...
  'This may happen if there is a temporary network anomaly on the server side, like cosmic radiation or spooky ghosts. If this failure was caused by a transient service interruption, this server will recover momentarily.' => 'Cela peut se produire en cas d’anomalie temporaire du réseau côté serveur. Si cette défaillance a été causée par une interruption de service passagère, le serveur se rétablira dans un instant.',
  ...
  'This server is currently configured with no writable ("master") database, so it can not write new information anywhere. This server will run in read-only mode until an administrator reconfigures it with a writable database.' => 'Ce serveur est actuellement configuré avec une base de données principale non modifiable aussi vous ne pouvez pas du tout écrire de nouvelles informations. Ce serveur fonctionnera en mode lecture seule jusqu’à ce qu’un administrateur le reconfigure avec une base de données accessible en écriture.',
  ...
  'Object "%s" exists, but does not have a URI to redirect to.' => 'L’objet « %s » existe mais n’a aucune URI auquel rediriger.',
